...
class MessagingInMemory[F[_], A](topic: Topic[F, A])(implicit F: Sync[F]) extends MessagingPublisher[F, A] {
...
object MessagingInMemory {
  def make[F[_], A]()(implicit F: Async[F]): F[MessagingInMemory[F, A]] = {
    Topic[F, A]
      .map(new MessagingInMemory[F, A](_))
  }
}
